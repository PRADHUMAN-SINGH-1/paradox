...
function metric(label: string, value: string | number, tone = ''): string {
  return '<article class="vx-metric ' + (tone ? 'vx-metric--' + tone : '') + '">' +
    '<span>' + escapeHtml(label) + '</span><strong>' + escapeHtml(String(value)) + '</strong></article>';
}
  ...
  const refs = claim.evidence.length
    ? claim.evidence.map((ref) =>
        '<div class="vx-evidence-ref">' +
          '<div><code>' + escapeHtml(ref.file) + ':' + ref.line + '</code>' +
          '<span>' + escapeHtml(ref.quote) + '</span></div>' +
        '</div>'
      ).join('')
    : '<div class="vx-evidence-ref vx-evidence-ref--empty">No validated file evidence attached.</div>';

  return '<article class="vx-claim vx-claim--' + claimTone(claim.status) + '">' +
    '<div class="vx-claim-head">' +
      '<div><span class="vx-claim-id">' + escapeHtml(claim.id) + '</span>' +
      '<h4>' + escapeHtml(claim.claim) + '</h4></div>' +
...
export function renderAnalysis(x: Analysis, opts: { compareHref?: string } = {}): string {
  ...
  const aiPanel = ai
    ? '<section class="vx-card vx-card--ai vx-adjudication-card">' +
        '<div class="vx-card-head">' +
          '<div><span class="vx-eyebrow">AGENT INVESTIGATION</span><h3>How the evidence was adjudicated</h3></div>' +
        ...
        '</div>' +
        '<footer class="vx-card-foot"><span>Provider: ' + escapeHtml(aiProvider) + ' · ' + escapeHtml(aiModel) + '</span>' +
          '<span>Adversarial evidence pass · no repository execution</span></footer>' +
      '</section>'
    : '<section class="vx-card vx-card--ai vx-card--idle vx-adjudication-card">' +
        '<div class="vx-card-head">' +
          '<div><span class="vx-eyebrow">AGENT INVESTIGATION</span><h3>Evidence adjudication</h3></div>' +
  ...
  const inspectedFiles = x.structure.map((p) =>
    '<code>' + escapeHtml(p) + '</code>'
  ).join('');

  return '<section class="vx-result vx-result--investigation" aria-label="Repository verification result">' +
    '<header class="vx-hero vx-hero--investigation">' +
      '<div class="vx-identity">' +
        '<div class="vx-kicker"><span>PARADOX VERIFY</span><span>ANALYSIS ' +
          escapeHtml(x.analyzedAt.replace('T', ' ').replace('Z', ' UTC')) + '</span></div>' +
        '<div class="vx-title-row">' +
          '<div><p class="vx-repo">' + escapeHtml(x.meta.fullName) + '</p><h2>' + escapeHtml(x.meta.name) + '</h2></div>' +
      ...
      '<aside class="vx-score-box vx-score-box--hero"><span>PARADOX SCORE</span>' +
        ...
        '<p>Supporting context, not a security certification.</p></aside>' +
    '</header>' +

    '<section class="vx-summary-strip" aria-label="Evidence summary">' +
      '<article class="vx-summary-item vx-summary-item--confirmed"><span>CONFIRMED</span><strong>' + confirmed + '</strong><small>claims with validated evidence</small></article>' +
      '<article class="vx-summary-item vx-summary-item--contradicted"><span>CONTRADICTED</span><strong>' + contradicted + '</strong><small>claims rejected by evidence</small></article>' +
      '<article class="vx-summary-item vx-summary-item--unconfirmed"><span>UNCONFIRMED</span><strong>' + unconfirmed + '</strong><small>claims needing review</small></article>' +
    ...
    '<section class="vx-decision-grid" aria-label="Verification decision">' +
      '<article class="vx-card vx-decision-card">' +
        '<div class="vx-card-head"><div><span class="vx-eyebrow">FINAL ADJUDICATION</span><h3>Why Verify reached this result</h3></div>' +
          '<span class="vx-decision-verdict">' + escapeHtml(verdictLabel(x.verdict)) + '</span></div>' +
        ...
        '</div>' +
      '</article>' +
      '<aside class="vx-card vx-context-card">' +
        '<div class="vx-card-head"><div><span class="vx-eyebrow">RUN CONTEXT</span><h3>Observable facts</h3></div></div>' +
        '<div class="vx-facts">' +
    ...
    '<section class="vx-prime-evidence" aria-label="Primary evidence">' +
      '<div class="vx-section-heading"><div><span class="vx-eyebrow">PRIMARY EVIDENCE</span><h3>Claim → file → evidence</h3>' +
        '<p>The evidence ledger is the primary verification surface. Every confirmed or contradicted claim must survive repository quote validation.</p></div>' +
        '<strong class="vx-count vx-count--large">' + claims.length + '</strong></div>' +
    ...
    '<section class="vx-grid vx-grid--findings" aria-label="Security and implementation findings">' +
      '<article class="vx-card vx-card--security">' +
        '<div class="vx-card-head"><div><span class="vx-eyebrow">SECURITY SURFACE</span><h3>Risk indicators</h3></div>' +
          '<strong class="vx-count vx-count--' + (highRisks ? 'risk' : 'good') + '">' + x.risks.length + '</strong></div>' +
        '<div class="vx-risk-summary"><span><b>' + highRisks + '</b> HIGH</span><span><b>' + moderateRisks + '</b> MODERATE</span>' +
          '<span><b>' + (x.risks.length - highRisks - moderateRisks) + '</b> LOW</span></div>' +
      ...
      '<article class="vx-card">' +
        '<div class="vx-card-head"><div><span class="vx-eyebrow">IMPLEMENTATION SIGNALS</span><h3>Models &amp; tools</h3></div>' +
          '<strong class="vx-count">' + x.detections.length + '</strong></div>' +
    ...
    '<section class="vx-secondary-grid" aria-label="Repository health context">' +
      '<article class="vx-card"><div class="vx-card-head"><div><span class="vx-eyebrow">REPOSITORY HEALTH</span><h3>Supporting metrics</h3></div></div>' +
        '<div class="vx-metrics vx-metrics--embedded">' + healthMetrics + '</div></article>' +
      '<article class="vx-card"><div class="vx-card-head"><div><span class="vx-eyebrow">PROJECT PROFILE</span><h3>Repository context</h3></div></div>' +
        '<div class="vx-facts">' +
    ...
    '<details class="vx-drawer"><summary><span>INSPECTED FILES</span><b>' + x.structure.length + '</b><i>OPEN</i></summary>' +
      '<div class="vx-file-grid">' + (inspectedFiles || '<span class="vx-empty">No file paths were returned.</span>') + '</div></details>' +
    '<details class="vx-drawer"><summary><span>README / PROJECT CONTEXT</span><b>UNTRUSTED TEXT</b><i>OPEN</i></summary>' +
      '<div class="vx-readme"><p>' + escapeHtml(x.readmeExcerpt || 'Unknown') + '</p><span>Languages: ' + langs + '</span></div></details>' +
