...
The fallback router uses provider-specific latency budgets and a concrete free OpenRouter model (`openai/gpt-oss-120b:free`) for more predictable structured JSON output. OpenRouter's `gpt-oss-120b:free` model supports structured outputs, while Hugging Face's OpenAI-compatible router supports `openai/gpt-oss-120b:fastest` for automatic fastest-provider selection.
