...
Verify now uses Gemini as the primary repository investigator through Gemini's native structured-output path when `GEMINI_API_KEY` is available. Planner, final investigator and critic stages use stage-specific reasoning levels and output budgets, while a routed multi-provider fallback is used only when Gemini cannot complete a stage. The successful fallback provider is carried into later stages instead of restarting the full provider cascade, reducing latency and model switching.
`AI_PROVIDER_ORDER` is treated as a provider preference rather than an allowlist: any other configured provider remains eligible for fallback. This prevents an incomplete preference setting from accidentally disabling the rest of the configured provider pool when a primary model is rate-limited or unavailable.
For Verify, when Gemini is unavailable, `analyze-repo` first attempts the OpenRouter free router directly with schema-constrained JSON, then falls back to the remaining server-side providers. This avoids losing the primary recovery path to router-level provider ordering or cooldown state while preserving deterministic evidence validation and adversarial adjudication.
Verify now preserves provider-exhaustion diagnostics from the AI router. When every eligible provider fails, the result records the attempted provider path and sanitized failure codes (such as quota, authentication, timeout, output, or unavailable) while keeping the deterministic verdict path authoritative. This makes provider configuration or quota failures observable instead of presenting an opaque `UNAVAILABLE` state.
The fallback router uses provider-specific latency budgets and a concrete free OpenRouter model (`openai/gpt-oss-120b:free`) for more predictable structured JSON output. OpenRouter's current model documentation lists this free model as supporting structured output, while Hugging Face documents `openai/gpt-oss-120b:fastest` for its OpenAI-compatible router.
...
The fallback router treats `AI_PROVIDER_ORDER` as a preference, keeps every configured provider eligible for fallback, uses a concrete free OpenRouter GPT-OSS model by default, and disables provider-specific JSON mode where compatibility is less predictable. The router validates returned JSON locally before accepting the provider response.
Verify delegates provider selection to the AI Router for each investigation stage rather than making a separate direct Gemini call first. This removes the duplicate Gemini wait when Gemini is rate-limited, keeps the router's provider health/cooldown logic authoritative, and preserves the successful-provider continuity for later stages.
For Verify-specific tasks, provider cooldown state from earlier requests is not allowed to suppress the preferred fallback pool on a new verification run. Each fresh Verify request gets a new two-provider attempt budget, preventing a transient provider failure in a previous run from hiding an otherwise configured fallback.
All Verify AI stages now share the same two-provider budget and repository-specific fallback order, including the final security-verification stage. This prevents the planner from using the general six-provider budget or the final stage from falling back to an unrelated provider order.
Verify now uses a three-provider free fallback budget for repository stages: Gemini first, then OpenRouter's free-model router, then Hugging Face's fastest GPT-OSS policy. Provider HTTP status is retained in sanitized failure classification so a failed fallback can be distinguished from quota, authentication, timeout and output-format errors. The routed response parser also accepts string and content-part response shapes used by OpenAI-compatible endpoints.
When a later Verify stage reuses a previously successful fallback provider, that provider is no longer accidentally excluded from its own request. If the reused provider fails, the stage explicitly retries the remaining fallback pool while excluding the known-failed provider and Gemini. This keeps provider continuity without turning a transient provider failure into an immediate `UNAVAILABLE` result.
For repository investigation tasks, the preferred fallback path now prioritizes `Gemini → Hugging Face → OpenRouter` before the remaining configured providers. Verify also carries the provider that successfully completed the previous stage into the next stage, and only retries through other providers when that active provider fails. The browser analysis cache version is bumped when this contract changes so stale AI-unavailable results are not reused.
