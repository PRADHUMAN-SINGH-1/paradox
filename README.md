...
The LLM is responsible for bounded repository reasoning and investigation. Deterministic code remains responsible for repository facts and evidence validation.
...
The fallback router uses provider-specific latency budgets and explicit current model defaults. OpenRouter uses `openai/gpt-oss-120b:free`, Hugging Face uses `openai/gpt-oss-120b:fastest`, Groq uses `openai/gpt-oss-20b`, Cerebras uses `gpt-oss-120b`, Mistral uses `mistral-small-latest`, NVIDIA uses `openai/gpt-oss-20b`, Cohere uses `command-a-plus-05-2026`, and Cloudflare uses `@cf/meta/llama-3.3-70b-instruct-fp8-fast`. These model IDs and endpoint shapes are aligned with the providers' current API documentation. citeturn520943search0turn898414search0turn898414search4turn163272search0turn163272search1turn163272search2turn450682search0
