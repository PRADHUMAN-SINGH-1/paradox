...
Verify does not prioritize Gemini or any single model provider. All AI investigation stages use the server-side AI Router with `provider: auto`; the router selects from the configured provider pool and skips unavailable or rate-limited providers. Deterministic evidence validation remains authoritative.
...
For Verify, `analyze-repo` uses the same neutral AI Router path for every investigation stage. Provider selection is rotated across the configured pool rather than starting from Gemini, and failed providers are excluded for the current request. This preserves deterministic evidence validation and adversarial adjudication while avoiding a single-provider dependency.
...
The Verify analyzer uses the server-side AI Router for repository investigation. Provider selection is sequential per request and rotates the starting provider so Gemini is not inherently preferred. Verify keeps its deterministic evidence rules authoritative and falls through when a provider is unavailable, rate-limited, times out, or returns invalid structured output. The successful provider, model and attempted provider path are returned in analysis metadata; failed providers enter a short server-side cooldown.

The fallback router treats `AI_PROVIDER_ORDER` as a preference, keeps every configured provider eligible for fallback, and validates provider JSON locally before accepting a response. Verify's primary stage calls Gemini directly; when Gemini is unavailable it attempts OpenRouter's free `gpt-oss-120b:free` endpoint directly without provider-specific JSON-mode requirements, then falls through to the remaining configured providers through the AI Router. The successful provider/model and attempted path are retained in the result metadata.
Verify-specific provider state is bounded per request and does not allow an old provider failure to suppress a fresh repository investigation. Provider HTTP status is retained in sanitized failure classification so quota, authentication, timeout, output-format, 4xx and 5xx failures can be distinguished.
